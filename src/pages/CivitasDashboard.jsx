...
import { Shield, MapPin, Phone, Mic, Navigation } from 'lucide-react';
...
export default function CivitasDashboard() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      // Fitur disembunyikan tapi komponen tetap ada di codebase
    ...
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col">
      ...
      <header className="hidden md:flex items-center justify-between px-8 py-4 bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-slate-200">
        ...
        <nav className="flex bg-slate-100 p-1 rounded-full">
           {/* HANYA MENAMPILKAN HOME DAN MAP */}
           {['home', 'map'].map(t => (
      ...
      {/* --- MOBILE NAVBAR (HANYA 2 MENU) --- */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-3 flex justify-center gap-12 z-50 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
...
const MapTab = () => (
  <div className="w-full h-[70vh] bg-slate-200 rounded-3xl overflow-hidden relative border border-slate-300">
    ...
    <div className="absolute top-4 left-4 bg-white/90 px-3 py-1 rounded-lg text-xs font-bold shadow-md">
       📍 Area Perguruan Tinggi
    </div>
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-red-600 text-white p-2 rounded-full shadow-xl animate-bounce">
